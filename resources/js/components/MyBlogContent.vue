<template>
    <div class="d-flex flex-wrap">
      <div v-for="blog in blogs" :key="blog.id" class="col-md-4" data-aos="flip-left">
        ...
        <!-- Modal -->
        <div class="modal fade" :id="'modal-' + blog.id" aria-labelledby="exampleModalLabel" aria-hidden="true" tabindex="-1">
          <div class="modal-dialog modal-xl">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <p>
                    <figcaption class="blockquote-footer">{{ blog.user.name }}{{ formatCreatedAt(blog.created_at) }}</figcaption>
                ...
                <img :src="getImageUrl(blog.image)" class="img-fluid imageBlog" alt="Blog Image" />
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-circleLibrary" data-bs-dismiss="modal"> Close </button>
              </div>
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    export default {
      ...
      methods: {
        fetchBlogs() {
          axios.get("/blogosphere").then((response) => {
